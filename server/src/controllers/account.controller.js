...
const {
  signUpSync,
  createNewPasswordSync,
  activeAccountSync,
  changeStatusAccountSync
} = require("../microservices/synchronize/account");
const mail = require("nodemailer");
const bcrypt = require("../helpers/secures/rcrypt");
...
module.exports = {
  ...
  changeStatus: async (req, res) => {
    const { id } = req.body,
      ...
      vpsContainServer = await Server.findOne({ userAmount: userInfo._id })
        .select("info")
        .lean();
    ...
    let data = await Account.findByIdAndUpdate(
      ...
      resUserSync = await changeStatusAccountSync(
        `${vpsContainServer.info.domainServer}:${vpsContainServer.info.serverPort}/api/v1/users/sync/change-status`,
        req.body,
  ...
  index: async (req, res) => {
    ...
    if (req.query._id) {
      dataResponse = await Account.findOne({ _id: req.query._id })
        .select("-password")
        .populate({ path: "_role", select: "_id level" })
        .lean();
      ...
      dataResponse = await Account.find(req.query)
        .select("-password")
        .lean();
      return res.status(200).json(jsonResponse("success", dataResponse));
    }
    ...
    if (req.query._size && req.query._page) {
      ...
      // Handle with mongodb
      dataResponse = await Account.find(
        {},
  ...
  searchUser: async (req, res) => {
    const userInfo = await Account.findOne({
        $or: [
          { email: req.query._keyword },
          { phone: req.query._keyword },
          { presenter: req.query._keyword }
        ]
      })
        .select("-password")
        .populate({ path: "_role", select: "_id level" })
        .populate({ path: "_server", select: "title" })
        .lean(),
      membershipInfo = await MembershipPackage.findOne({
  ...
  renewById: async (req, res) => {
    ...
    if (req.body.id === undefined || req.body.id.length === 0) {
      return res.status(403).json({
        status: "fail",
        id: "Vui lòng cung cấp id người dùng để kích hoạt!"
      });
    } else if (req.body.expireDate < new Date()) {
      return res.status(403).json({
        status: "fail",
        expireDate: "Ngày gia hạn cần phải ở tương lai!"
      });
    }
    ...
    if (!userInfo) {
      return res
        .status(404)
        .json({ status: "error", message: "Người dùng này không tồn tại!" });
    }
    ...
    data = await Account.findByIdAndUpdate(
      id,
      {
        $set: { status: 1, expireDate: expireDate, maxAccountFb: maxAccountFb }
      },
      { new: true }
    ).select("-password");

    resUserSync = await activeAccountSync(
      `${vpsContainServer.info.domainServer}:${vpsContainServer.info.serverPort}/api/v1/users/active`,
      req.body,
      req.headers.authorization
    );
    if (resUserSync.data.status !== "success") {
  ...
  renewAutoUsingCode: async (req, res) => {
    ...
    if (!findCode) {
      return res
        .status(403)
        .json({ status: "error", message: "Mã code này không tồn tại!" });
    }
    // Check code expire
    if (new Date(findCode.expireDate) < new Date()) {
      return res
        .status(400)
        .json({ status: "error", message: "Mã code này đã hết hạn sử dụng!" });
    }
    if (findCode.numberOfUser >= findCode.maxUser) {
      return res.status(406).json({
        status: "error",
        message: "Mã code này đã đạt tối đa số lượt sử dung!"
      });
    }
    ...
    if (userInfo.code && userInfo.code === req.body.code) {
      return res.status(405).json({
        status: "error",
        message: "Bạn đã từng sử dụng mã code này, vui lòng nhập mã khác!"
      });
    }
    ...
    data = await Account.findByIdAndUpdate(
      req.uid,
      {
        $set: {
          status: 1,
          expireDate: new Date(
            date.setMonth(date.getMonth() + findCode.typeExpire)
          ),
          code: req.body.code
        }
      },
  ...
  renewByCode: async (req, res) => {
    ...
    if (req.body.presenter === undefined || req.body.presenter.length === 0) {
      return res.status(403).json({
        status: "fail",
        presenter: "Mã kích hoạt không được để trống"
      });
    }
    ...
    if (req.body.presenter && req.body.presenter.length > 0) {
      ...
      if (userList.length === 0) {
        return res
          .status(404)
          .json({ status: "error", message: "Mã kích hoạt không tồn tại!" });
      }
      ...
      await Promise.all(
        userList.map(async (user) => {
          ...
          resUserSync = await activeAccountSync(
            `${vpsContainServer.info.domainServer}:${vpsContainServer.info.serverPort}/api/v1/users/active`,
            { id: id, expireDate: req.body.expireDate },
            req.body.headers
          );
          if (resUserSync.data.status !== "success") {
            return res.status(404).json({
  ...
  signIn: async (req, res) => {
    ...
    let userInfo = await Account.findOne({
      email: email.toString().toLowerCase()
    });

    if (!userInfo) {
      userInfo = await Account.findOne({
        phone: email.toString().toLowerCase()
      });
    }
    const memberRole = await Role.findOne({ _id: userInfo._role });
    const serverContainUser = await Server.findOne({
      userAmount: userInfo._id
    });
    ...
    if (new Date(userInfo.expireDate) < new Date()) {
      ...
      return res.status(405).json({
        status: "error",
        message:
          "Tài khoản của bạn đã hết hạn. Vui lòng liên hệ với bộ phận CSKH!"
      });
    }
    cookie = `sid=${signToken(userInfo)}; uid=${userInfo._id}; cfr=${
      memberRole.level
    };`;
    ...
    res.status(201).json(
      jsonResponse("success", {
        ...
        domain:
          process.env.APP_ENV === "production" ? `${serverContainUser.info.domain}/` : `${serverContainUser.info.domain}:${serverContainUser.info.clientPort}/`
      })
    );
  },
  signUp: async (req, res, next) => {
    let { name, email, phone, password, presenter, region } = req.body,
      ...
      optimalServer = await Server.findOne({ region, status: 1 }).sort({
        slot: -1
      });
    ...
    if (phoneRegex.test(phone) === false) {
      return res.status(403).json({
        status: "error",
        message: "Số điện thoại phải có 10 chữ số và phải bắt đầu bằng số 0."
      });
    }
    ...
    if (emailRegex.test(email) === false) {
      return res
        .status(403)
        .json({ status: "error", message: "Địa chỉ email không hợp lệ!" });
    }
    ...
    if (name.length > 50 || name.length < 6) {
      return res.status(403).json({
        status: "error",
        message: "Tên không được ngắn hơn 6 ký tự hoặc dài hơn 50 ký tự!"
      });
    }
    if (nameRegex.test(name) === false) {
      return res.status(403).json({
        status: "error",
        message: "Tên không được chứa các ký tự đặc biệt!"
      });
    }
    // #endregion
    ...
    if (pingResult.alive === false) {
      return res.status(403).json({
        status: "error",
        message: "Tên miền của email server không có thực!"
      });
    }
    // #endregion

    const freeAccountPackage = await MembershipPackage.findOne({
      codeId: "free"
    });
    ...
    if (resSyncNestedServer.data.status !== "success") {
      return res
        .status(404)
        .json({ status: "error", message: "Quá trình đăng ký xảy ra vấn đề!" });
    }
    ...
    cookie = `sid=${signToken(newUser)}; uid=${newUser.id}; cfr=${
      memberRole.level
    };`;
    res.set("Cookie", cookie);
    ...
    if (
      req.headers.authorization && req.headers.authorization.includes("aid") && findSubString(req.headers.authorization, "aid=", ";") !== "None"
    ) {
      const findAgency = await Agency.findOne({
        _id: findSubString(req.headers.authorization, "aid=", ";")
      });
    ...
    res.status(201).json(
      jsonResponse("success", {
        ...
        domain:
          process.env.APP_ENV === "production" ? `${optimalServer.info.domain}/` : `${optimalServer.info.domain}:${optimalServer.info.clientPort}/`
      })
    );
  },
  signUpAccountBackup: async (objectData) => {
    const {
      ...
      isPhoneExist = await Account.findOne({ phone }),
      memberRole = await Role.findOne({ level: "Member" }),
      optimalServer = await Server.findOne({ region: 0, status: 1 }).sort({
        slot: -1
      }),
  ...
  resetPassword: async (req, res, next) => {
    ...
    if (!email) {
      return res
        .status(404)
        .json({ status: "error", message: "Vui lòng cung cấp email!" });
    }
    ...
    if (!serverContainUser) {
      return res.status(404).json({
        status: "error",
        message:
          "Tài khoản này hiện tại không còn khả dụng trên hệ thống! Vui lòng liên hệ với bên bộ phận CSKH"
      });
    }
    ...
    linkReset += `reset-password/final?authorization=${cryptoRandomString({
      length: 50,
      type: "url-safe"
    })}&timestamp=${cryptoRandomString({
      length: 10,
      characters: "1234567890"
    })}&serverToken=${serverContainUser.info.domain}&token=${signToken(
      userInfo
    )}&__a=1`;
    ...
    res.status(200).json({
      status: "success",
      data: `Vui lòng check email ${req.body.email} để lấy code`
    });
  },
  renewPasswordByAdmin: async (req, res) => {
    ...
    if (email) {
      ...
      if (!userInfo) {
        if (phoneNumber) {
          userInfo = await Account.findOne({ phone: phoneNumber });
        }
      }
    } else if (phoneNumber) {
      userInfo = await Account.findOne({ phone: phoneNumber });
    ...
    userInfo = await Account.findById(userInfo._id);
    return res.status(200).json({
      status: "success",
      data: userInfo
    });
  ...
  createNewPassword: async (req, res) => {
    ...
    if (!userInfo) {
      return res.status(404).json({
        status: "error",
        message: "Phiên tạo mới mật khẩu của bạn đã hết hạn!"
      });
    }

    // Sync
    resUserSync = await createNewPasswordSync(
      `${vpsContainServer.info.domainServer}:${vpsContainServer.info.serverPort}/api/v1/users/create-password`,
      { password },
      {
        Authorization: `sid=${signToken(userInfo._id)}; uid=${
          userInfo._id
        }; cfr=${memberRole.level};`
      }
    );
    if (resUserSync.data.status !== "success") {
      return res.status(404).json({
    ...
    res
      .status(201)
      .json(jsonResponse("success", "Tạo mới mật khẩu thành công!"));
  },
  getUserInfoLostPass: async (req, res) => {
    const userInfo = await Account.findOne({ email: req.query.email }).select(
      "name email imageAvatar"
    );
  ...
  signInByAdmin: async (req, res) => {
    ...
    res
      .status(201)
      .json(jsonResponse("success", `${userInfo.email} đăng nhập thành công!`));
  },
  signUpByAdmin: async (req, res) => {
    ...
    if (
      key.staffKey.includes(req.headers.token) === false && key.collaboratorKey.includes(req.headers.token) === false
    ) {
      return res.status(404).json({
        status: "error",
        message:
          "Mã xác thực nhân viên của bạn không đúng hoặc IP không hợp lệ!"
      });
    }
    ...
    if (name === undefined || name.length === 0) {
      return res
        .status(403)
        .json({ status: "fail", name: "Tên không được để trống!" });
    } else if (phone === undefined || phone.length === 0) {
      return res
        .status(403)
        .json({ status: "fail", phone: "Số điện thoại không được để trống!" });
    } else if (email === undefined || email.length === 0) {
      return res
        .status(403)
        .json({ status: "fail", email: "Email không được để trống!" });
    } else if (password === undefined || password.length === 0) {
      return res
        .status(403)
        .json({ status: "fail", password: "Mật khẩu không được để trống!" });
    }
    ...
    header = `sid=${signToken(newUser)}; uid=${newUser._id}; cfr=${
      adminRole.level
    }`;
    ...
    res
      .status(201)
      .json(jsonResponse("success", `${newUser.email} đăng ký thành công!`));
  },
  updateSync: async (req, res) => {
    ...
    if (!userInfo) {
      res.send({
        status: "error",
        message: "Tài không được đồng bộ trên server!"
      });
    }
